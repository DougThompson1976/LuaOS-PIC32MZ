...
// ==========================================================================
// 2012 - Modified by Bill Wilson to output font files in a format suitable
// for my ukrellm microcontroller libglcd library.
//
// Compile with:
//		cc -o bdf2c -Wall bdf2c.c
//
// glcd library changes from original:
//		o Added option to do tables only if asked.  Don't need tables
//		  for my microcontroller fixed width font application.
//
//		o Changed 'break' to 'continue' when empty lines are encountered.
// ==========================================================================
...
void CreateFontHeaderFile(FILE * out)
{
	...
	"\tunsigned short n_chars;\t\t///< number of characters in font\n"
	...
	"// (c) 2009, 2010 Lutz Sammer, License: AGPLv3\n\n"
	"struct GlcdFont\n\t{\n"
	"\tunsigned char char_width;\t\t///< max. character width\n"
	"\tunsigned char char_height;\t\t///< character height\n"
	"\tunsigned short n_chars;\t\t///< number of characters in font\n"
	"\tconst unsigned char *bitmap;\t///< bitmap of all characters\n"
	"\n\t};\n\n");
		}
...
void Header(FILE * out, const char *name)
{
    ...
    fprintf(out,
	"static const unsigned char __%s_bitmap__[] __FLASH__ = {\n", name);
}
...
void Footer(FILE * out, const char *name, int width, int height, int chars)
{
    ...
    fprintf(out,
	"GlcdFont %s =\n\t{\n", name);
    fprintf(out, "\t.char_width = %d,\n", width);
    fprintf(out, "\t.char_height = %d,\n", height);
    fprintf(out, "\t.n_chars = %d,\n", chars);
    fprintf(out, "\t.first_char = %s,\n", "(uint8) ' '");
	if (do_tables)
		{
...
///
void DumpCharacter(FILE * out, unsigned char *bitmap, int width, int height, int bbx, int bby, int bbw, int bbh, int ascent, int descent)
{
    int x;
    int y;
    int c;

    for (y = bbh; y < ascent - bby; ++y) {
			fputc('\t', out);
		for (x = 0; x < width; x += 8) {
			c = 0;
            fprintf(out, "%02x:  ", c);
    ...
    for (y = 0; y < height - ascent + bbh + bby; ++y) {
	fputc('\t', out);
	for (x = 0; x < width; x += 8) {
...
void ReadBdf(FILE * bdf, FILE * out, const char *name)
{
	...
	if (!strcasecmp(s, "FONT_ASCENT")) {
		p = strtok(NULL, " \t\n\r");
		ascent = atoi(p);
	}

	if (!strcasecmp(s, "FONT_DESCENT")) {
		p = strtok(NULL, " \t\n\r");
		descent = atoi(p);
	}
	...
	if (!strcasecmp(s, "STARTCHAR")) {
		...
		fontboundingbox_height, bbx, bby, bbw, bbh, ascent, descent);
	    scanline = -1;
	    width = INT_MIN;
	} else {
	    if (scanline >= 0) {
...
int main(int argc, char *const argv[])
{
	...
	switch (getopt(argc, argv, "bcC:n:hO?-")) {
		...
		Outline = 1;
		continue;
		case 't':		/* Do tables only if asked */
			do_tables = 1;
			continue;
	    case EOF:
		break;
