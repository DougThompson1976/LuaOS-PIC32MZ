...
#define CORE_TIMER_USEC_STEPS ((CPU_HZ / 2) / 1000000)
...
void platform_timer_delay( unsigned id, timer_data_type delay_us )
{
  ...
  if( id == PLATFORM_TIMER_SYS_ID )
  {
    ...
    if( delay_us > 0 )
    {
      delay_us = delay_us * CORE_TIMER_USEC_STEPS;
      
      tstart = platform_timer_read_sys();
      tend = tstart;
      ...
      while( 1 )
      {
        if( ( tend = platform_timer_read_sys() ) < tstart ) { // overflow
            if (first) {
                tend += ( u64 )(PLATFORM_TIMER_SYS_MAX - tstart) + 1;
                first = 0;
            } else {
