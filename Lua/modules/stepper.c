...
static void lstepper_end(void *arg1, uint32_t arg2) {
    lstepp[arg2].done = 1;
}
...
static int lstepper_setup( lua_State* L ) {
    ...
    if ((error = steppers_setup(pulse_width))) {
...
static int lstepper_move( lua_State* L ){
    ...
    stepper_move(lstepper->unit + 1, dir, steps, ramp, ifreq, efreq, lstepper_end);
