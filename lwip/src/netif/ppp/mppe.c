...
mppe_init(ppp_pcb *pcb, ppp_mppe_state *state, u8_t options)
{
	...
	if (options & MPPE_OPT_128)
		...
		PPPDEBUG(PPP_LOG_DEBUG, ("%s[%d]: unknown key length\n", debugstr,
			pcb->netif->num));
...
#if PPP_DEBUG
	{
		...
		PPPDEBUG(PPP_LOG_DEBUG, ("%s[%d]: initialized with %d-bit %s mode\n",
...
mppe_compress(ppp_pcb *pcb, ppp_mppe_state *state, struct pbuf **pb, u16_t protocol)
{
	...
	PPPDEBUG(PPP_LOG_DEBUG, ("mppe_compress[%d]: ccount %d\n", pcb->netif->num, state->ccount));
	...
	if (!state->stateful ||	/* stateless mode     */
		...
		/* We must rekey */
		if (state->stateful) {
			PPPDEBUG(PPP_LOG_DEBUG, ("mppe_compress[%d]: rekeying\n", pcb->netif->num));
		}
		mppe_rekey(state, 0);
		state->bits |= MPPE_BIT_FLUSHED;
	}
...
mppe_decompress(ppp_pcb *pcb, ppp_mppe_state *state, struct pbuf **pb)
{
	...
	PPPDEBUG(PPP_LOG_DEBUG, ("mppe_decompress[%d]: ccount %d\n",
	...
	if (!state->stateful && !flushed) {
		PPPDEBUG(PPP_LOG_DEBUG, ("mppe_decompress[%d]: FLUSHED bit not set in "
		       "stateless mode!\n", pcb->netif->num));
		state->sanity_errors += 100;
		sanity = 1;
	}
	if (state->stateful && ((ccount & 0xff) == 0xff) && !flushed) {
		PPPDEBUG(PPP_LOG_DEBUG, ("mppe_decompress[%d]: FLUSHED bit not set on "
		       "flag packet!\n", pcb->netif->num));
		state->sanity_errors += 100;
		sanity = 1;
	}
